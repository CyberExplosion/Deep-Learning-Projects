...
class Method_MNIST(method, nn.Module):
    ...
    def __init__(self, mName='', mDescription='', sData=None):
        ...
        self.classifier = OrderedDict([
            ...
            # ('dropout_layer_3', nn.Dropout(p=self.data['sDropout'], inplace=True)),

            ('linear_layer_3', nn.Linear(5408, 100)),
            ('activation_func_3', nn.ReLU()),
        ])
        self.outputLayer = OrderedDict([
            ('linear_layer_4', nn.Linear(100, self.data['sOutputDim']))
        ])
...
# 
#             ('avgpool_layer_6', nn.AdaptiveAvgPool2d(6))
#         ])   # add more layers later
#         self.classifier = OrderedDict([
#             ('transitional_flat_layer', nn.Flatten()),
# 
#             ('dropout_layer_7', nn.Dropout(p=self.data['sDropout'])),
#             ('linear_layer_7', nn.Linear(9216, 4096)),
...
#         ])
        ...
        if self.data['sLossFunction'] != 'CrossEntropy':
            ...
            self.outputLayer['output'] = nn.ReLU(inplace=True)
    ...
    def trainModel(self, X, y):
        ...
        for epoch in range(self.data['sMaxEpoch']):
            ...
            for i in range(0, len(X), self.data['sBatchSize']):
                ...
                batchX, batchY = [X[i] for i in indices], [y[i] for i in indices]
    ...
    def run(self):
        ...
        netStats = summary(self, (1,1,28,28))
